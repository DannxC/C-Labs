...
/* Escrita amigavel em video-texto */
    ...
    printf("Analise de Vetor e Matriz");
    printf("\n\n\tMatriz de entrada no arquivo: input.txt");
    printf("\n\n\tMatriz de saida e todos os resultados obtidos no arquivo: output.txt");
...
/* Montagem da tabela: elementos de A; media dos quadrados dos elementos de cada linha; quantidade de termos impares de cada linha */
    ...
    for(i = 0; i <= m-1; i++)
    {
        ...
        for(j = 0, aux = 0, impar[i] = 0; j <= n-1; j++)
        {
            ...
            aux += A[i][j] * A[i][j];           //Variavel auxiliar para o calculo da media aritmetica dos quadrados
...
/* Quantidade de termos maiores que zero por coluna */
    ...
    for(j = 0; j <= n-1; j++)
    {
        ...
        {
            if(A[i][j] > 0)
                cont[j]++;                      //Contador dos numeros positivos da coluna j
        }
...
/* Menor elemento de cada coluna */
    ...
    for(j = 0; j <= n-1; j++)
    {
        for(i = 0, menor[j] = A[0][j]; i <= m-1; i++)
            if(A[i][j] < menor[j])
                menor[j] = A[i][j];             //Escolha do menor numero da coluna j
...
/* Escrita de B no arquivo de saida */
    ...
    for(i = 0; i <= m-1; i++)
    {
        fprintf(FileOut, "\nLinh: %d", i);
        for(j = 0, aux = 0, impar[i] = 0; j <= n-1; j++)
            fprintf(FileOut, "%8d", B[i][j]);           //Escrita dos elementos de B no arquivo de saida
    }
    fprintf(FileOut, "\n-------------------------------------------------------");
